...
public class SearchingInAnArray {
    ...
    public static boolean linearSearch(int[] array, int length, int element){
        // Check for edges cases. Is the array null or empty?
        // If it is, then we return false because the elements we're searching
        // for couldn't possibly be in it.
        if(array == null|| length == 0){
            return false;
        }

        //Carry out the linear search by checking each element, starting from
        // the first one:
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Does the array contain the element 4? - " + SearchingInAnArray.linearSearch(array, length, 4));
        System.out.println("Does the array contain the element 30? - " + SearchingInAnArray.linearSearch(array, length, 30));
